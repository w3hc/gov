...
async function main() {
    ...
    console.log("Creating proposal to update voting delay...")

    try {
        const newVotingDelay = 42n
        const targets = [gov.target]
        const values = [0]
        const calldatas = [
            gov.interface.encodeFunctionData("setVotingDelay", [newVotingDelay])
        ]
        const description = "Update voting delay to 24 hours " + Date.now()
                ...
                while (!isSucceeded) {
                    ...
                    if (getProposalState(Number(state)) === "Succeeded") {
                        ...
                        try {
                            ...
                            // Verify the voting delay was updated
                            try {
                                ...
                                console.log(
                                    "Could not verify voting delay update:",
                                    error
                                )
                            }
