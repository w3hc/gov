import { ethers } from "hardhat";
const color = require("cli-color")
var msg = color.xterm(39).bgXterm(128);
import * as store from '../store.json'
const hre = require("hardhat");
const fs = require('fs');

async function main() {
  
  // deployer = 0x70456d078950db075283931D9bE2E01B49f3e71e = "Goerli Super tester" addr

  // const SSD = await ethers.getContractFactory("SSD")
  // const ssd = await SSD.deploy(store.sugar)
  // await ssd.deployed();
  // console.log("Governor contract address:", msg(ssd.address), "✅")  

  // console.log("Etherscan verification in progress...")
  // await ssd.deployTransaction.wait(6)
  await hre.run("verify:verify", { network: "goerli", address: "0xdA29B7D299e3a6A77f1ceB2fABC83399ABFc14B8", constructorArguments: [store.sugar], });
  console.log("Etherscan verification done. ✅")

  fs.writeFileSync(
    "store.json",
    JSON.stringify({
      sugar: store.sugar, 
      ssd: "0xdA29B7D299e3a6A77f1ceB2fABC83399ABFc14B8"
    }, undefined, 2),
  ); 
}

main().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
