...
describe("Hypercerts", function () {
  ...
  describe("Interactions", function () {
    ...
    it("Should mint 2 * 50% of an hypercert", async function () {
      ...
      const call2 = await hypercerts.interface.encodeFunctionData('safeMint', [gov.address])
      const call3 = await hypercerts.interface.encodeFunctionData('safeMint', [gov.address])
    ...
    it("Should mint 2 hypercerts: 50% to gov, 50% to vault", async function () {
        const { gov, vault, alice, bob, erc20Mock, hypercerts } = await loadFixture(deployContracts);
        ...
        const call3 = await hypercerts.interface.encodeFunctionData('safeMint', [alice.address])
        const calldatas = [call.toString(), call2.toString(), call3.toString()]
      ...
      it("Should sell 1 hypercert", async function () {
